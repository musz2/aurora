  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        {/* Report toolbar — share + export grouped in one surface */}
        <div className="flex items-center overflow-hidden rounded-xl border border-black/[0.08] bg-white">
          <button
            onClick={() => share.mutate()}
            disabled={share.isPending}
            className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-ink transition hover:bg-black/[0.03] disabled:opacity-50"
          >
          ...
          <select
            ...
            className="bg-transparent py-2.5 pl-3 pr-1 text-sm text-ink outline-none"
...
function ChatPanel({ meetingId }: { meetingId: string }) {
  ...
  return (
    <Card className="flex h-[560px] flex-col">
      ...
      <form onSubmit={submit} className="border-t border-black/[0.06] p-3">
        <div className="flex items-center gap-2 rounded-2xl border border-black/[0.08] bg-white py-1 pl-3 pr-1 transition focus-within:border-aurora-400 focus-within:ring-2 focus-within:ring-aurora-100">
          <input
            value={input}
          ...
          <button
            type="submit"
            aria-label="Send"
            className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-ink text-white transition hover:bg-black active:scale-95"
          >
